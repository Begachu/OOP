...
Number *Complex::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		Integer* value_num = dynamic_cast<Integer*>(num);
		Complex* return_num = new Complex(real()+value_num->val(), imag());
		delete value_num;
		delete(this);
		return return_num;
    } else if (num->types() == FLOAT) {
		Float* value_num = dynamic_cast<Float*>(num);
		Complex* return_num = new Complex(real()+value_num->val(), imag());
		delete value_num;
		delete this;
		return return_num;
    } else if (num->types() == COMPLEX) {
		Complex* value_num = dynamic_cast<Complex*>(num);
		Complex* return_num = new Complex(real()+value_num->real(), imag()+value_num->imag());
		delete value_num;
		delete this;
		return return_num;
	}
...
Number *Complex::mul(Number * num) {
    ...
    if (num->types() == INTEGER) {
		Integer* value_num = dynamic_cast<Integer*>(num);
		Complex* return_num = new Complex(real()*value_num->val(), imag()*value_num->val());
		delete value_num;
		delete(this);
		return return_num;
    } else if (num->types() == FLOAT) {
		...
		Complex* value_num = dynamic_cast<Complex*>(num);
		Complex* return_num = new Complex(real()*value_num->real()-imag()*value_num->imag(), real()*value_num->imag()+imag()*value_num->real());
		delete value_num;
		delete this;
		return return_num;
	}
