...
먼저 위와 같은 target의 상수를 표현하기 위해서는 최소 13bit의 메모리가 필요하다.
이때  메모리 주소의 할당은 메모리 공간 절약보다는 탐색 속도에 초점이 맞춰져있어
4의 배수 혹은 8의 배수로 주소를 할당하기 때문에 16bit-13bit=3bit의 빈공간이 앞에 발생한다.
따라서 계산 시 추가로 빈공간 3bit의 값을 더해야 알맞은 결과가 나온다.
또한 long형도 빠른 데이터 탐색 속도를 위해 16의 배수 단위로 이를 저장하기 때문에
long형의 배열 b에도 8bit의 빈공간이 발생하게 되므로 이 또한 포함시켜야한다.
